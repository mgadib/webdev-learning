  ...
  return (
    <div className="mt-6 p-4 sm:p-6 rounded-2xl border border-dashed border-app-accent bg-app-surface-card transition-normal">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="font-body font-bold text-[15px] sm:text-[16px] text-app-accent">
          Eksplorasi dengan AI
        </h3>
      </div>

       <blockquote className="font-body text-[14px] sm:text-[15px] text-app-body italic border-l-4 border-app-accent pl-4 py-1 mb-4 bg-app-elevated/50 rounded-r-lg">
         <MarkdownContent content={prompt} className="inline-block" />
       </blockquote>

      <div className="flex flex-col sm:flex-row flex-wrap gap-2">
        <a href={chatgptUrl} target="_blank" rel="noopener noreferrer"
          className="inline-flex items-center justify-center gap-2 px-5 py-2.5 border border-app-default text-app-muted rounded-full font-body font-medium text-[13px] sm:text-[14px] transition-normal hover:border-app-strong hover:text-app-heading min-h-[44px]"
        >
