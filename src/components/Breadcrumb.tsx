...
export default function Breadcrumb({ items }: BreadcrumbProps) {
  return (
    <nav className="mb-6">
      <ol className="flex items-center gap-1 text-[12px] sm:text-[13px] font-body text-app-muted flex-wrap">
        {items.map((item, i) => (
          <li key={i} className="flex items-center gap-1">
            ...
            {item.path ? (
              ...
              <span className="font-semibold text-app-heading truncate max-w-[150px] sm:max-w-[300px]">
                {item.label}
              </span>
            )}
          </li>
